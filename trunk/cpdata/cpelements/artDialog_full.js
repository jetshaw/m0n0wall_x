...
function art_rand_string (len){
	...
	if(hash.length<len)hash+=art_rand_string(len-hash.length);
	return hash;
}
function _close_window_one(){
	$(".art_dialog_append_dom:first").remove();
	$(".d_mask_dialog_window:last").remove();
}
function ______close_window_one____(){
	$(".art_dialog_append_dom:first").remove();
	$(".d_mask_dialog_window:last").remove();
}
function _find_art_first_id(){
	return jQuery('.d-content:last').attr('id');
}
function _find_art_first_title(){
	return jQuery('.dialog_append_dom_tilte:last').attr('id');	
}
function append_content_to_window(url,content){
	var dd ='#'+_find_art_first_id();
	if(url){
		var obj = $.ajax({
			type:'GET',
...
$.fn = $.prototype = {
    ...
    */
    html: function (content) {
	   ...
	   var elem = this[0];
	   if (content === undefined) return jQuery(elem).html();
	  
	   if(!jQuery(elem).attr('id')){
		  jQuery(elem).attr({"id":art_rand_string(15)}).addClass('dialog_append_dom_tilte');   
	   }
...
artDialog.fn = artDialog.prototype = {
    ...
    constructor: function (config) {
        ...
        //$(".d-title").html(config.title);
		var d = $("#"+_find_art_first_title());
		$(d).html(config.title);
    ...
    */
    content: function (message) {
        var prev, next, parent, display,parentL;
        var that = this,
            $content = this.dom.content,
            content = $content[0];
		    parentL = content.parentNode;
			content.style.width = parentL.style.width;
			content.style.height = parentL.style.height;
//			content.style.overflow = "auto";
		...
		return this	
        /*if(count++ % 2 == 1){console.log(123)
			jQuery(".art_dialog_append_dom").css("display","block");
			return this.position()
		}else{jQuery(".art_dialog_append_dom").css("display","none");
			 return this;}*/
    },
    ...
    */
    ...
    lock: function () {
        ...
        $div.css({
            ...
            height: '100%',
            overflow: 'hidden'
        }).addClass('d-mask d_mask_dialog_window');
    ...
    _getDom: function () {
		...
		//var i = art_rand_string(10);
        jQuery(wrap).attr({'id':art_rand_string(10),'class':'art_dialog_append_dom'});
...
+							'<tr>'
+								'<td class="d-footer">'
+									'<div id="d-buttons" class="d-buttons"></div>'
+								'</td>'
+							'</tr>'
+						'</tbody>'
+					'</table>'
