...
# php 4.4.9
cd $MW_BUILDPATH/tmp
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/bin/php" ]; then
...
# mini httpd
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/mini_httpd" ]; then
	cd $MW_BUILDPATH/tmp
	...
	patch < $MW_BUILDPATH/freebsd8/build/patches/packages/mini_httpdc.patch
    make
	install -s mini_httpd $MW_BUILDPATH/m0n0fs/usr/local/sbin
fi
# ezipupdate
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/bin/ez-ipupdate" ]; then
	cd $MW_BUILDPATH/tmp
...
if [ ! -f "$MW_BUILDPATH/m0n0fs/sbin/ipf" ]; then #|| [ ! -f "$MW_BUILDPATH/m0n0fs/sbin/ipfs" ] || [ ! -f "$MW_BUILDPATH/m0n0fs/sbin/ipfstat" ] || [ ! -f "$MW_BUILDPATH/m0n0fs/sbin/ipmon" ] || [ ! -f "$MW_BUILDPATH/m0n0fs/sbin/ipnat" ] || [ ! -f "$MW_BUILDPATH/m0n0fs/sbin/ippool" ] ]; then
	cd $MW_BUILDPATH/tmp
...
# modem-stats
if [ ! -f "$MW_BUILDPATH/m0n0fs/sbin/modem-stats" ]; then
	cd $MW_BUILDPATH/tmp
...
# dnsmasq
if [ ! -f "MW_BUILDPATH/m0n0fs/usr/local/sbin/dnsmasq" ]; then
	cd $MW_BUILDPATH/tmp
	rm -rf dnsmasq-2.66
...
######## FreeBSD ports ########
# ISC dhcp-server
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/dhcpd" ]; then
...
# ISC dhcp-client
if [ ! -f "$MW_BUILDPATH/m0n0fs/sbin/dhclient" ]; then
...
# ISC dhcp-relay
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/dhcrelay" ]; then
...
# ipsec-tools
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/racoon" ] || [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/setkey" ] || [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/libipsec.so.0" ]; then
...
# dhcp6
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/dhcp6c" ] || [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/dhcp6s" ]; then
...
# sixxs-aiccu		
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/sixxs-aiccu/aiccu" ]; then
...
# mpd5
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/mpd5" ]; then
...
# mbmon
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/bin/mbmon" ]; then
...
# wol
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/bin/wol" ]; then
	cd $PORTSDIR/net/wol
	make WITHOUT_NLS=true
	install -s $WRKDIRPREFIX/$PORTSDIR/net/wol/work/wol-*/src/wol $MW_BUILDPATH/m0n0fs/usr/local/bin/
fi

#cp $MW_BUILDPATH/freebsd8/build/tools/sshd $MW_BUILDPATH/m0n0fs/usr/local/sbin
#cp $MW_BUILDPATH/freebsd8/build/tools/ssh-keygen $MW_BUILDPATH/m0n0fs/usr/local/sbin
#cp "$MW_BUILDPATH/freebsd8/build/tools/grep" $MW_BUILDPATH/m0n0fs/usr/local/sbin
#cp "$MW_BUILDPATH/freebsd8/build/tools/more" $MW_BUILDPATH/m0n0fs/usr/local/sbin
#cp /usr/bin/whoami $MW_BUILDPATH/m0n0fs/usr/local/sbin
chmod a+x $MW_BUILDPATH/m0n0fs/usr/local/sbin

# make m0n0wall tools and binaries
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/stats.cgi" ] ||
     [  ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/minicron"  ] ||
     [  ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/choparp"   ] ||
     [  ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/verifysig" ] ||
     [  ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/dnswatch"  ] ||
     [  ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/voucher"   ] ||
     [  ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/croen" ]; then
	cd $MW_BUILDPATH/tmp
...
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/runsntp.sh" ]; then
	install "$MW_BUILDPATH/freebsd8/build/tools/runsntp.sh" $MW_BUILDPATH/m0n0fs/usr/local/bin
fi
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/ppp-linkup" ]; then
	install "$MW_BUILDPATH/freebsd8/build/tools/ppp-linkup" $MW_BUILDPATH/m0n0fs/usr/local/sbin
fi
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/vpn-linkdown" ]; then
	install "$MW_BUILDPATH/freebsd8/build/tools/vpn-linkdown" $MW_BUILDPATH/m0n0fs/usr/local/sbin
fi
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/vpn-linkup" ]; then
	install "$MW_BUILDPATH/freebsd8/build/tools/vpn-linkup" $MW_BUILDPATH/m0n0fs/usr/local/sbin
fi

# select Autoconf version 2.62
export AUTOCONF_VERSION=2.62
# ucd-snmp
if [ ! -f "$MW_BUILDPATH/m0n0fs/usr/local/sbin/snmpd" ]; then
	cd $MW_BUILDPATH/tmp
